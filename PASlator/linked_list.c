...
SList *slist_prepend(SList *list, Pointer data){
	SList *result = (SList *)smalloc(sizeof(SList));
	if (result == NULL){
		return list;
	}
	result->data = data;
...
SList *slist_remove_if(SList *list, Pointer userdata, int(*compare_func)(Pointer listdata, Pointer userdata)){
	...
	for (SList *i = list; i != NULL; i = next){
		next = i->next;
		if (compare_func(i->data, userdata) == 1){
			if (last != NULL)
				last->next = i->next;
			if (i == list)
				list = list->next;
...
SList *slist_find_custom(SList *list, Pointer needle, int(*compare_func)(Pointer listdata, Pointer needle)){
	for (SList *i = list; i != NULL; i = i->next){
		int compare_result = (*compare_func)(i->data, needle);
		if (compare_result == -1)
			return NULL;
		if (compare_result == 0)
			return i;
...
typedef struct _findcustom{
	int(*compare_func)(Pointer listdata, Pointer needle);
	Pointer needle;
	SList *result;
} FindCustom;

void findall(Pointer data, FindCustom *helper){
	if (helper->compare_func(data, helper->needle) == 0){
		helper->result = slist_prepend(helper->result, data);
	}
}
...
SList *slist_find_all_custom(SList *list, Pointer needle, int(*compare_func)(Pointer listdata, Pointer needle)){
	FindCustom helper;
	helper.compare_func = compare_func;
	helper.needle = needle;
	helper.result = NULL;
	slist_foreach(list, findall, &helper);
